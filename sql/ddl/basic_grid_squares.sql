...
CREATE TABLE
  basic_grid_squares
  (
    basic_grid_square_code  char(8)           NOT NULL PRIMARY KEY
...
COMMENT ON TABLE basic_grid_squares IS '基準地域メッシュ';

-- set the logical name of the columns
COMMENT ON COLUMN basic_grid_squares.basic_grid_square_code
  IS '基準地域メッシュコード';
COMMENT ON COLUMN basic_grid_squares.second_grid_square_code
  IS '２次メッシュコード';
COMMENT ON COLUMN basic_grid_squares.geometry
  IS '基準地域メッシュの領域';
