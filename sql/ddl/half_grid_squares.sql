...
CREATE TABLE
  ...
  , standard_grid_square_code char(8)           NOT NULL
  , geometry                  geometry(polygon) NOT NULL
  , FOREIGN KEY (standard_grid_square_code)
      REFERENCES standard_grid_squares (standard_grid_square_code)
);
...
COMMENT ON COLUMN half_grid_squares.half_grid_square_code
  IS '２分の１地域メッシュコード';
COMMENT ON COLUMN half_grid_squares.standard_grid_square_code
  IS '基準地域メッシュコード';
